...
function includesArgs(parameters) {
  ...
  if (!args)
    return false;
  function isEqual2(input2, value, arg) {
    try {
  ...
  if (Array.isArray(args) && Array.isArray(matchArgs)) {
    return matchArgs.every((value, index2) => {
      ...
      return value_.some((value2) => isEqual2(input2, value2, args[index2]));
    });
  }
  if (typeof args === "object" && !Array.isArray(args) && typeof matchArgs === "object" && !Array.isArray(matchArgs))
    return Object.entries(matchArgs).every(([key, value]) => {
      ...
      if (!input2)
        return false;
      const value_ = Array.isArray(value) ? value : [value];
      return value_.some((value2) => isEqual2(input2, value2, args[key]));
    });
  return false;
}
...
var [SHA3_IOTA_H3, SHA3_IOTA_L3] = /* @__PURE__ */ split3(_SHA3_IOTA3, true);
...
var Keccak3 = class _Keccak extends Hash4 {
  // NOTE: we accept arguments in bytes instead of bits here.
  constructor(blockLen, suffix, outputLen, enableXOF = false, rounds = 24) {
    super();
    this.blockLen = blockLen;
...
var gen3 = (suffix, blockLen, outputLen) => wrapConstructor(() => new Keccak3(blockLen, suffix, outputLen));
var sha3_224 = /* @__PURE__ */ gen3(6, 144, 224 / 8);
var sha3_256 = /* @__PURE__ */ gen3(6, 136, 256 / 8);
var sha3_384 = /* @__PURE__ */ gen3(6, 104, 384 / 8);
var sha3_512 = /* @__PURE__ */ gen3(6, 72, 512 / 8);
var keccak_224 = /* @__PURE__ */ gen3(1, 144, 224 / 8);
var keccak_2563 = /* @__PURE__ */ gen3(1, 136, 256 / 8);
var keccak_384 = /* @__PURE__ */ gen3(1, 104, 384 / 8);
var keccak_512 = /* @__PURE__ */ gen3(1, 72, 512 / 8);
var genShake = (suffix, blockLen, outputLen) => wrapXOFConstructorWithOpts((opts = {}) => new Keccak3(blockLen, suffix, opts.dkLen === void 0 ? outputLen : opts.dkLen, true));
var shake128 = /* @__PURE__ */ genShake(31, 168, 128 / 8);
var shake256 = /* @__PURE__ */ genShake(31, 136, 256 / 8);

// ../runtime/src/canonical.ts
var SCHEME_ID = "lockstep-skill-hash/v2";
var DOMAIN_LEAF = 0;
var DOMAIN_ROOT = 1;
var NORMALISED_EXTENSIONS = /* @__PURE__ */ new Set([
  ".md",
  ".markdown",
  ".txt",
  ".js",
...
function shouldNormalise(path) {
  return NORMALISED_EXTENSIONS.has(extensionOf(path));
}
function normaliseTextBytes(input2) {
  let start = 0;
  if (input2.length >= 3 && input2[0] === 239 && input2[1] === 187 && input2[2] === 191) {
    start = 3;
  }
  const out = new Uint8Array(input2.length - start);
  let written = 0;
  for (let i = start; i < input2.length; i += 1) {
    const byte = input2[i];
    if (byte === 13) {
      out[written] = 10;
      written += 1;
      if (input2[i + 1] === 10) i += 1;
      continue;
    }
    out[written] = byte;
    written += 1;
...
function canonicalisePath(rawPath) {
...
function isExcluded(canonicalPath) {
  const [top] = canonicalPath.split("/");
  return top !== void 0 && EXCLUDED_TOP_LEVEL.has(top);
}
...
function hashLeaf(canonicalPath, content, executable = false) {
...
function hashSkill(files) {
  ...
  for (const file of files) {
    ...
    const content = normalised ? normaliseTextBytes(file.content) : file.content;
  ...
  const entries = [...seen.values()].sort(
    (a, b) => comparePathBytes(a.path, b.path)
  );
  ...
  return {
    skillHash: toHex3(keccak_2563(concat3(parts))),
    scheme: SCHEME_ID,
    entries
  };
}
...
async function isExecutable(absolutePath) {
  const stat = await lstat(absolutePath);
  return (stat.mode & 73) !== 0;
}
async function loadSkillFiles(root, options = {}) {
  const files = [];
...
// ../runtime/src/hashDirectory.ts
async function hashSkillDirectory(root, options = {}) {
...
function computeVersionId(name, version4) {
  return keccak256(encodeAbiParameters([{ type: "string" }, { type: "string" }], [name, version4]));
}
var MANIFEST_FILENAME = "lockstep.json";
function parseSelector(raw, where) {
  ...
  if (/^0x[0-9a-fA-F]{8}$/.test(raw)) {
    return { selector: raw.toLowerCase(), label: raw.toLowerCase() };
  }
...
function parseManifest(json) {
  ...
  const capabilities = rawCalls.map((entry, i) => {
...
async function loadManifest(skillDir, explicitPath) {
  const path = explicitPath ?? join2(skillDir, MANIFEST_FILENAME);
  let raw;
  try {
...
var HIGH_RISK_LABELS = Object.fromEntries([
  ...SIGNATURES.map(([signature, label]) => [toFunctionSelector(signature), label]),
  // Empty calldata: a bare native-value send to an arbitrary address.
  ["0x00000000", "bare native value transfer to any address"]
]);
function isHighRiskSelector(selector) {
...
`);
  ...
  if (unlocked < bond) {
    fail(
      `Insufficient unlocked bond: need ${formatUnits(bond, 6)}, have ${formatUnits(unlocked, 6)}. Deposit more, or narrow the manifest \u2014 a narrower manifest is cheaper by design.`
    );
  }
